...
class Encoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.conv1 = nn.Conv1d(1, 64, kernel_size=5, stride=2, padding=1)  #  : 1x800,  : 64x400
        self.conv2 = nn.Conv1d(64, 32, kernel_size=5, stride=2, padding=1)  #  : 64x400,  : 32x200
        self.conv3 = nn.Conv1d(32, 16, kernel_size=5, stride=2, padding=2)  #  : 32x200,  : 16x100
        self.conv4 = nn.Conv1d(16, 4, kernel_size=5, stride=2, padding=2)   #  : 16x100,  : 4x50
        
        self.fc_mu = nn.Linear(4 * 50, latent_dim)  
        self.fc_logvar = nn.Linear(4 * 50, latent_dim)

    def forward(self, x):
        (batch_size, 1, 800)
        if len(x.shape) == 2:  #  (batch_size, 800)
            x = x.unsqueeze(1)  #  (batch_size, 1, 800)
        x = F.relu(self.conv1(x))  #  (batch_size, 64, 400)
        x = F.relu(self.conv2(x))  #  (batch_size, 32, 200)
        x = F.relu(self.conv3(x))  #  (batch_size, 16, 100)
        x = F.relu(self.conv4(x))  #  (batch_size, 4, 50)
        x = x.view(x.size(0), -1)  # : (batch_size, 4 * 50)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.fc = nn.Linear(latent_dim, 4 * 50)  #  (batch_size, 4 * 50)
        
        self.deconv1 = nn.ConvTranspose1d(4, 16, kernel_size=5, stride=2, padding=2, output_padding=1)  #  : (batch_size, 16, 100)
        self.deconv2 = nn.ConvTranspose1d(16, 32, kernel_size=5, stride=2, padding=2, output_padding=1) #  : (batch_size, 32, 200)
        self.deconv3 = nn.ConvTranspose1d(32, 64, kernel_size=5, stride=2, padding=2, output_padding=1) #  : (batch_size, 64, 400)
        self.deconv4 = nn.ConvTranspose1d(64, 1, kernel_size=5, stride=2, padding=2, output_padding=1)  #  : (batch_size, 1, 800)

    def forward(self, z):
        x = self.fc(z)  # (batch_size, 4 * 50)
        x = x.view(x.size(0), 4, 50)  #(batch_size, 4, 50)
        x = F.relu(self.deconv1(x))  #  : (batch_size, 16, 100)
        x = F.relu(self.deconv2(x))  #  : (batch_size, 32, 200)
        x = F.relu(self.deconv3(x))  # : (batch_size, 64, 400)
        x = torch.sigmoid(self.deconv4(x))  #  (batch_size, 1, 800)
        return x.squeeze(1)  #  (batch_size, 800)
