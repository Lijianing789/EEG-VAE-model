...
def normalization(data):
    ...
    normalization_data =(data - data_min) / (data_max - data_min+1e-12)
...
class Encoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.conv1 = nn.Conv1d(1, 64, kernel_size=5, stride=2, padding=1)  # 输入: 1x800, 输出: 64x400
        self.conv2 = nn.Conv1d(64, 32, kernel_size=5, stride=2, padding=1)  # 输入: 64x400, 输出: 32x200
        self.conv3 = nn.Conv1d(32, 16, kernel_size=5, stride=2, padding=2)  # 输入: 32x200, 输出: 16x100
        self.conv4 = nn.Conv1d(16, 4, kernel_size=5, stride=2, padding=2)   # 输入: 16x100, 输出: 4x50
        
        self.fc_mu = nn.Linear(4 * 50, latent_dim)  # 平展后再连接全连接层
        self.fc_logvar = nn.Linear(4 * 50, latent_dim)

    def forward(self, x):
        # 确保输入为三维张量 (batch_size, 1, 800)
        if len(x.shape) == 2:  # 如果输入为 (batch_size, 800)
            x = x.unsqueeze(1)  # 添加一个通道维度，变成 (batch_size, 1, 800)
        x = F.relu(self.conv1(x))  # 输出: (batch_size, 64, 400)
        x = F.relu(self.conv2(x))  # 输出: (batch_size, 32, 200)
        x = F.relu(self.conv3(x))  # 输出: (batch_size, 16, 100)
        x = F.relu(self.conv4(x))  # 输出: (batch_size, 4, 50)
        x = x.view(x.size(0), -1)  # 展平, 输出: (batch_size, 4 * 50)
        mu = self.fc_mu(x)  # 输出: (batch_size, latent_dim)
        logvar = self.fc_logvar(x)  # 输出: (batch_size, latent_dim)
        return mu, logvar
# 定义解码器 (Decoder)
# 定义解码器 (Decoder)
class Decoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.fc = nn.Linear(latent_dim, 4 * 50)  # 解码器的全连接层，输出: (batch_size, 4 * 50)
        
        self.deconv1 = nn.ConvTranspose1d(4, 16, kernel_size=5, stride=2, padding=2, output_padding=1)  # 输出: (batch_size, 16, 100)
        self.deconv2 = nn.ConvTranspose1d(16, 32, kernel_size=5, stride=2, padding=2, output_padding=1) # 输出: (batch_size, 32, 200)
        self.deconv3 = nn.ConvTranspose1d(32, 64, kernel_size=5, stride=2, padding=2, output_padding=1) # 输出: (batch_size, 64, 400)
        self.deconv4 = nn.ConvTranspose1d(64, 1, kernel_size=5, stride=2, padding=2, output_padding=1)  # 输出: (batch_size, 1, 800)

    def forward(self, z):
        x = self.fc(z)  # 输出: (batch_size, 4 * 50)
        x = x.view(x.size(0), 4, 50)  # 重塑为卷积层的输入格式: (batch_size, 4, 50)
        x = F.relu(self.deconv1(x))  # 输出: (batch_size, 16, 100)
        x = F.relu(self.deconv2(x))  # 输出: (batch_size, 32, 200)
        x = F.relu(self.deconv3(x))  # 输出: (batch_size, 64, 400)
        x = torch.sigmoid(self.deconv4(x))  # 输出: (batch_size, 1, 800)
        return x.squeeze(1)  # 去掉通道维度，返回形状 (batch_size, 800)


class Model(nn.Module):   # C_VAE
...
def train_vae():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (noisy_segments, clean_segments) in enumerate(train_loader):
            ...
            reconstructed_train_segment = bandpass_filtering(reconstructed_train_segment, 1,50, 200, 8)
 ...
 #%%validation   
        ...
        with torch.no_grad():
            for batch_idx, (noisy_segments, clean_segments) in enumerate(val_loader):
                ...
                reconstructed_val_segment = bandpass_filtering(reconstructed_val_segment, 1,50, 200, 8)
    ...
    with torch.no_grad():
        for batch_idx, (noisy_segments, clean_segments) in enumerate(tqdm(test_loader)):
           ...
           # reconstructed_test_segment = replace_jumps_with_mean(reconstructed_test_segment, 0.3)
            ...
            reconstructed_test_segment = bandpass_filtering(reconstructed_test_segment, 1,50, 200, 8)
...
def show_image_constrast(original, reconstructed, idx, title):
    ...
    for i, idx in enumerate(indices):
        plt.subplot(2, 4, i + 1)  
        plt.plot(time_axis, original[idx].flatten(), label='Clean EEG input', color='#1f77b4')
        plt.plot(time_axis, reconstructed[idx].flatten(), label='Clean EEG reconstruction', color='#ff7f0e', linestyle='--')
...
if __name__ == '__main__':
    ...
    aa=np.zeros(x_test_noisy.shape)
    aa=x_test_clean
    
    bb=np.zeros(x_test_clean.shape)
    bb=x_test_noisy
...
#%% Show images for some indices in train and test data
    ...
    for i in indices_to_show:
        show_images(train_dataset.clean_segments, reconstructed_train_data_epoch, i, 'Train')
        show_images(test_dataset.clean_segments, all_test_reconstructed_segments, i, 'Test')
